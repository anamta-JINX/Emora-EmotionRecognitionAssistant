...
function AccordionItem({ item, open, onToggle }) {
  return (
    <div className={`overflow-hidden rounded-[24px] border bg-white transition duration-300 ${open ? 'border-moss-300 shadow-card' : 'border-moss-100 hover:border-moss-200'}`}>
      <button type="button" onClick={onToggle} className="flex w-full items-center justify-between gap-5 p-5 text-left sm:p-6" aria-expanded={open}>
        <span className="font-display text-base font-extrabold tracking-[-0.02em] text-ink sm:text-lg">{item.question}</span>
        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full transition duration-300 ${open ? 'rotate-180 bg-ink text-white' : 'bg-moss-50 text-moss-700'}`}><ChevronDown size={17} /></span>
      </button>
      <div className={`grid transition-all duration-300 ${open ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
        <div className="overflow-hidden"><p className="border-t border-moss-100 px-5 py-5 text-sm leading-7 text-slate-600 sm:px-6">{item.answer}</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <PageHero
        ...
        description="Learn how the current EMORA project handles images, camera access, model setup, interpretation, and privacy."
      />

      <section className="pb-24 sm:pb-28">
        <div className="container-page grid gap-10 lg:grid-cols-[.65fr_1.35fr] lg:items-start">
          <Reveal className="lg:sticky lg:top-28">
            <div className="rounded-[30px] bg-ink p-7 text-white sm:p-8">
              <span className="grid h-14 w-14 place-items-center rounded-2xl bg-white/10 text-moss-200"><HelpCircle size={24} /></span>
              <h2 className="mt-6 font-display text-2xl font-extrabold tracking-[-0.04em]">Need a practical answer?</h2>
              <p className="mt-4 text-sm leading-7 text-slate-300">Most setup problems come from camera permissions, an unavailable Flask server, or a missing model file.</p>
              <Link to="/home#workspace" className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-ink transition hover:-translate-y-1"><Sparkles size={16} /> Test the workspace</Link>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-2">
              {[Camera, ImageUp, LockKeyhole].map((Icon, index) => <span key={index} className="grid h-20 place-items-center rounded-2xl border border-moss-100 bg-white text-moss-600"><Icon size={22} /></span>)}
            </div>
          </Reveal>

          <div className="space-y-3">
            {faqs.map((item, index) => (
              <Reveal key={item.question} delay={(index % 4) * 50}>
                <AccordionItem item={item} open={active === index} onToggle={() => setActive(active === index ? -1 : index)} />
              </Reveal>
            ))}
          </div>
        </div>
